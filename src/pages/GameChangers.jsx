...
const GameChangers = () => {
  ...
  const fetchTeamData = async () => {
    try {
      // Fetch players for game changers
      ...
      // Filter players and coaches for game changers
      const gameChangersPlayers = playersData.filter(p => 
        p.team === 'game-changers' && !p.role.toLowerCase().includes('coach')
      );
      const gameChangersCoaches = playersData.filter(p => 
        p.team === 'game-changers' && p.role.toLowerCase().includes('coach')
      );

      setPlayers(gameChangersPlayers);
      setCoaches(gameChangersCoaches);
      ...
      const upcoming = matchesSnapshot.docs
        ...
        .filter(m => m.date?.toDate() > now && m.team === 'game-changers')
        .sort((a, b) => a.date?.toDate() - b.date?.toDate())[0];

      setUpcomingMatch(upcoming);
      setLoading(false);
    } catch (error) {
      console.error('Error fetching game changers data:', error);
      setLoading(false);
    }
  };

  if (loading) {
    return <div className="loading">Loading Game Changers...</div>;
  }

  return (
    <div className="team-page game-changers">
      {/* Section 1: Team Header with Name, Socials, and Upcoming Game */}
      <section className="team-hero-section">
        <div className="team-hero-content">
          <h1 className="team-title">
            <TitleTypingAnimation text="GAME CHANGERS" speed={50} delay={100} />
          </h1>
          <p className="team-tagline">Innovation, Growth & The Future of Esports</p>
