  ...
  const fetchTeamData = async () => {
    try {
      ...
      const upcoming = matchesSnapshot.docs
        ...
        .filter(m => m.date?.toDate() > now && m.team === 'main')
        .sort((a, b) => a.date?.toDate() - b.date?.toDate())[0];

      setUpcomingMatch(upcoming);
      setLoading(false);
    } catch (error) {
      console.error('Error fetching main team data:', error);
      setLoading(false);
    }
  };

  if (loading) {
    return <div className="loading">Loading Main Team...</div>;
  }

  return (
    <div className="team-page main-team">
      {/* Section 1: Team Header with Name, Socials, and Upcoming Game */}
      <section className="team-hero-section">
        <div className="team-hero-content">
          <h1 className="team-title">
            <TitleTypingAnimation text="MAIN TEAM" speed={50} delay={100} />
          </h1>
          <p className="team-tagline">The Core Competitive Roster</p>
      ...
      {/* Section 2: Players and Coaches (copied from homepage) */}
